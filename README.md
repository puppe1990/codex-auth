...
| Command | Description |
|---------|-------------|
| `codex-auth list [--refresh-all]` | List all accounts, optionally refreshing all quotas first |
| `codex-auth refresh` | Refresh the latest quota/usage snapshot for all stored accounts |
...
Refresh all stored accounts first, then print the updated table:
...
Refreshes the current usage snapshot for every stored account via the ChatGPT usage API, then you can run `codex-auth list` to inspect the updated 5h and weekly windows.
